...
public:
  Node * parent;
	...
	Node() {};
	Node(int v, int m, Node * p)	{ value = v; move = m; parent = p; };
...
void bfs(int s, int g, const set<int> & forbid, bool verbose)
{
  ...
  // keep searching the queue while it isn't empty, haven't exceed max nodes and haven't found the goal
	while(!q.empty() && node_count < 1000 && !goal)	{
  	current = q.front(); q.pop(); // pop the node of the front of the queue
...
int main(int argc, char *argv[])
{
  ...
  if (!puzzlefile) {
	...
	// parse the file 
  puzzlefile.getline(tnum,4); start = atoi(tnum);
  puzzlefile.getline(tnum,4); goal = atoi(tnum);
